...
public class StringExample1 {
	...
	public static void main(String[] args) {
		...
		String hobby2 = "여행";
		//문자열 리터럴이 동일할 경우에는 String 객체를 공유하게 된다
		//test = hobby; 대입 후에 test == hobby2와 비교했을때 동등 비교가 가능합니다.
		...
		//문자열에서 equals()는 문자열 그자체를 비교하기 때문에 동등비교가 가능함
		System.out.println(test.equals(hobby3));
		//문자열은 equals() 사용 ,==사용 x 
		...
		System.out.println(str2.substring(7,12)); //끝 인덱스 번화 전까지

		//5.split(): 문자열을 원하는 기준(문자 또는 문자열)으로 자른다.
		str = "apple/banna/kiwi/lemon/orange";
		String[] fruit = str.split("/");
		System.out.println(Arrays.toString(fruit));
		// ()에는 반드시 기준 문자 혹은 코드열 코드가 있어야 함.
		
		//6.contains() : 해당 문자가 문자열에 포함되어 잇는지 여부를 체크
		// 포함되면 true, 그렇지 않으면 false
	    ...
	    System.out.println("lastIndexOf(\"d\") : " +str.indexOf('d'));
	    
	    //두번재 "d"를 indexOf를 통해서 확인하는 방법
	    System.out.println("indexof(\"d\")의 두번재 위치 : "+str.indexOf('d',4));
	    System.out.println("indexof(\"d\")의 세번재 위치 : "+str.indexOf('d',6));
		...
		String email = "bcd1234@NaVeR.CoM";
		str = "naver.com";

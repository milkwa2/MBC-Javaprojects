...
public interface RemoteControl {
	...
	// 상수 필드(멤버변수) : 인터페이스 내에 멤버변수(필드)설정을 하면 이것은 상수 값이 됩니다.
	// public static final 이 생략됨....
	int MAX_VOLUME = 10; 
	int MIN_VOLUME = 0;
	
	// 추상 메서드 -> 구현측에서 반드시 재정의해야 함.
	//1. 반환타입, 2. 매개변수 타입과 객수를 지정
	void turnOn();     // public abstract 생략...
	void turnOff();
	void setVolume(int volume);
	
	// 디폴트 메서드 -> {}를 사용하여 코드 실행부분이 존재함.
	// 키워드로 default 를 사용함. 상수 필드를 읽거나 추상 메서드를 호출하는 코드를 작성할 수 잇음.
	// 무음...(mute) 버튼
	default void setMute(boolean mute) {
		if(mute) {
			System.out.println("무음 처리를 합니다.");
			setVolume(MAX_VOLUME); //위에 있는 추상 메서드를 호출...
		}else {
	...
	// 선언식 public 생략햇다면, 자동으로 컴파일 과정 중에 public이 붙게 됨.

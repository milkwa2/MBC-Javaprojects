...
public class StaticExample1 {
	...
	//정적 멤버 변수(필드) 및 메서드 => static
	// 정적 멤버 변수 -> 전역변수(프로그램 전체에 영향을 주는 변수) 설정과 비슷함.
	// 전역변수는 프로그램 시작과 동시에 선언되어서 사용되거나 혹은 중간에 선언되어도 프로그램 종료까지 사용됨.
	// 전연변수와 반대되는 지역변수 {} (블럭)내에 선언되어지는 변수. 블럭을 벗어나면 사라져요.
...
//		return ++a;
		return ++new Count().a; //인스턴스 생성후 a멤버 변수 참조
//	    return ++b;           // static을 이용한 클래스 멤버변수

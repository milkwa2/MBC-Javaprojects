...
public class MethodExample1 {
   // 메서드
         ...
         /*
          *       (접근제한자) (static) 반환타입 메서드명(메개변수타입 매개변수명) {
          *       
          *          여러 기능 구현 코드들...
          * 
          *       }
          ...
          *       매개변수 : 메서드(함수) 호출 시에 ()안에 함꼐 전달할 값을 받는 변수
   ...
   static int[] getLottoNumber() {
      ...
      final int LOTTO_SIZE = 6; // final - 한번 선언된 내용은 변경X
      ...
      for (int i = 0 ; i< LOTTO_SIZE; i++) {
         ...
         for (int j = 0; j < i; j++) {
            if(lotto[j] == lotto[i]) {
               i --;       // 중복이 있기 떄문에... 저장 공간 값을 줄여서 다시 실행...
               break;
            }
         }
      }
   ...
   // 토끼 이모티콘 출력 내용을 createRabit() 메서드로 console하는 메서드 구현...
   static void createRabit() {  // 매개변수 X, 반환 타입X
      System.out.println("====================");
      System.out.println("\t/)/)");
      System.out.println("       (  ..)");
      System.out.println("       (  >♡");
   ...
   // 메시지를 전달하면, 해당 메시지 " * * * " 값을 양쪽에 붙여주는 문자열을만드는 메서드...
   // 매개값 전달 : String
   // 반환값 : String
   ...
   public static void main(String[] args) {
      ...
      // 문자열을 넣으면 ***을 붙여서 반환하는 함수 : make Banner("문자열")

...
public class MapExample {
	...
	/*
	 *  Map
	 *  - key-alue 쌍으로 만들어지는 데이터
	 *  - key을 이용해서 value을 참조하며  작업
	...
	public static void main(String[] args) {
		...
		map.put("ABC", 123);
		map.put("FF", 123);
		map.put("홍길동", 88);  // 이미 있는 키에 값을 넣는 경우, 덮어쓴다(갱신)
...
//		System.out.println(map);
		...
		// countainsKey(key) : key가 포함되어 있는지 여부 확인
		System.out.println(map.containsKey("홍길동"));
		System.out.println(map.containsKey(10));
		
		// countainsValue(value) : value가 존재하는지 여부 확인
		System.out.println(map.containsValue(88));
		
		// remove(key) : 키 값으로 Map.Entry를 삭제하고, 그값을 반환
		System.out.println("삭제후 반환값 : " +map.remove("홍길동"));
		System.out.println(map);
